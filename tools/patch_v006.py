...
# ---------- UI ----------
ui_path = ROOT / "app/src/main/java/com/euclab/app/AppUiV5.kt"
ui = ui_path.read_text(encoding="utf-8")
ui = ui.replace("v0.0.5", "v0.0.6")
...
private fun V5WheelModelBadge(model: String?, online: Boolean) {
    ...
    val edge = if (online) V5Accent.copy(alpha = 0.45f) else Color(0xFF343B46)
    Box(
        modifier = Modifier
            .size(42.dp)
...
if "private fun buildVeteranCommandOld" not in ble:
    marker = '    @SuppressLint("MissingPermission")\n    private fun sendCommand(bytes: ByteArray): Boolean {'
    helpers = r'''    private fun buildVeteranCommandOld(
        cmdByte: Int,
        valuePosition: Int,
        value: Int,
        byte5: Int = 0x01,
...
'''
    if marker not in ble:
        raise RuntimeError("sendCommand marker not found")
...
print("EUC Lab v0.0.6 patch applied")
