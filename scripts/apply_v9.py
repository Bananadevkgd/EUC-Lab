...
# v0.0.9 is intentionally layered on top of the already field-tested v0.0.8 patch.
...
# Human-facing LeaperKim names. The official app calls these controls
# Acc/Dec Speed Helper, Acceleration Reduction and Stop Power Rate.
replace_once(
    wheel,
...
# Deep discharge is deliberately isolated with a persistent red warning.
replace_once(
    wheel,
...
# Service controls: make the risk explicit and remove the unverified brake-pressure control from normal UI.
replace_once(
    wheel,
...
replace_once(
    ...
    '// Brake Pressure Alarm is intentionally hidden: command/readback are known, but the physical meaning of the percentage is not yet documented well enough for a safe user-facing control.',
)

# Version text shown on the dashboard and package metadata.
